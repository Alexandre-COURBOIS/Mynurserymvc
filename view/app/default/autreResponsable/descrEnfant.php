...
<div class="table-responsive" style="width: 80%;margin: 20px 10%;">
    <table class="table table-striped">
        ...
        <tbody>
        <tr>
            <th><?= strtoupper($_SESSION['login']['nom']); ?></th>
        ...
        if (!empty($autreResp)) {
            foreach ($autreResp as $autreRes) {
                ...
                <tr>
                    <th><?= strtoupper($autreRes->nom_autre_respons); ?></th>
                    <td><?= ucfirst($autreRes->prenom_autre_respons); ?></td>
                    <td><?= ucfirst($role); ?></td>
                    <td><?= '<a id="btnDelete" href="' . $view->path('deleteResponsable', array($enfant[0]->id_enfant, $autreRes->id_autre_responsable)) . '">Supprimer ce responsable</a>'; ?></td>
                </tr>
           <?php
            }
        } ?>
